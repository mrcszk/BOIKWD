...
class Model:
    ...
    def _create_dual_variables(self, primal, dual):
        for x in range(len(primal.constraints)):
            dual.create_variable(f"y{x}")

    def _create_dual_objective(self, primal, dual):
        factors = []
        for x in range(len(primal.constraints)):
            factors.append(primal.constraints[x].bound)
        dual.minimize(primal.constraints[0].expression.from_vectors(dual.variables, factors))

    def _create_dual_constraints(self, primal, dual):
        table = np.array([c.expression.factors(primal) for c in primal.constraints]).T
        for i in range(len(table)):
            expr = va.Variable('', -1)
            for j in range(len(table[i])):
                expr = expr.__add__(table[i][j] * dual.variables[j])
            dual.add_constraint(expr >= primal.objective.expression.atoms[i].factor)
